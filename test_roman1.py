...
class KnownValues(unittest.TestCase):
    ...
    def test_from_roman_know_values(self):
        """from_roman should give know result with known input"""
        for integer, numeral in self.known_values:
...
class ToRomanBadInput(unittest.TestCase):
    def test_to_large(self):
        """to_toman should fail with large input"""
    ...
    def test_negative(self):
        """to_roman should failt with nefagative input"""
        self.assertRaises(roman1.OutOfRangeError, roman1.to_roman, -1)

    def test_non_integer(self):
        """to_toman should fail with non-integer input"""
        self.assertRaises(roman1.NotIntegerError, roman1.to_roman, 0.5)


class RaudtripCheck(unittest.TestCase):
    def test_roudtrip(self):
        """form_roman(to_roman(n)) == n for all n"""
...
class FromRomanBadInput(unittest.TestCase):
    def test_too_many_repeated_numerals(self):
        """form_roman should fail with too many repeated numerals"""
        for s in('MMMM', 'DD', 'CCCC', 'LL', 'XXXX', 'VV', 'IIII'):
            self.assertRaises(roman1.InvalidRomanNumeralError, roman1.from_roman, s)

    def test_repeated_pairs(self):
        """form_roman should fail with repeated pairs of numerals"""
        for s in ('CMCM', 'CDCD', 'XCXC', 'XLXL', 'IXIX', 'IVIV'):
            self.assertRaises(roman1.InvalidRomanNumeralError, roman1.from_roman, s)

    def test_malformed_antecedents(self):
        """form_roman should fail with malformed antecedents"""
